...
private:
	...
	inline void FluxF_Ground(typeAxis<1>){
		...
		F.resize(GeneralEquation<Tprec>::u.ubound(firstDim),
			 	 GeneralEquation<Tprec>::u.ubound(secondDim) + 1);
		F = u.P()*u.P();
	}
	...
	inline void FluxF_Ground(typeAxis<2>){
		...
		inline void FluxG_Ground(typeAxis<2>){
			...
			G.resize(GeneralEquation<Tprec>::v.ubound(firstDim) + 1,
					GeneralEquation<Tprec>::v.ubound(secondDim));
			G = v.P()*v.P();
...
:GeneralEquation<Tprec>(A,B,C),u(UT),v(VT),d(delta)
{
}
